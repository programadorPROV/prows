Refinery::I18n.frontend_locales.each do |lang|
  ...
  Refinery::User.find_each do |user|
    user.plugins.where(name: 'refinerycms-pv_board_directors').first_or_create!(
      position: (user.plugins.maximum(:position) || -1) +1
    )
  end if defined?(Refinery::User)

  Refinery::Page.where(link_url: (url = "/pv_board_directors")).first_or_create!(
    title: 'Pv Board Directors',
    deletable: false,
    menu_match: "^#{url}(\/|\/.+?|)$"
  ) do |page|
