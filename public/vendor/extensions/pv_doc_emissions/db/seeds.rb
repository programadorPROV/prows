Refinery::I18n.frontend_locales.each do |lang|
  ...
  Refinery::User.find_each do |user|
    user.plugins.where(name: 'refinerycms-pv_doc_emissions').first_or_create!(
      position: (user.plugins.maximum(:position) || -1) +1
    )
  end if defined?(Refinery::User)

  Refinery::Page.where(link_url: (url = "/pv_doc_emissions")).first_or_create!(
    title: 'Pv Doc Emissions',
    deletable: false,
    menu_match: "^#{url}(\/|\/.+?|)$"
  ) do |page|
