Refinery::I18n.frontend_locales.each do |lang|
  ...
  Refinery::User.find_each do |user|
    user.plugins.where(name: 'refinerycms-pv_interest_links').first_or_create!(
      position: (user.plugins.maximum(:position) || -1) +1
    )
  end if defined?(Refinery::User)

  Refinery::Page.where(link_url: (url = "/pv_interest_links")).first_or_create!(
    title: 'Pv Interest Links',
    deletable: false,
    menu_match: "^#{url}(\/|\/.+?|)$"
  ) do |page|
