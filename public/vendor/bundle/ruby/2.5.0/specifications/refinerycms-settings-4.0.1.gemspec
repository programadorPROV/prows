# -*- encoding: utf-8 -*-
# stub: refinerycms-settings 4.0.1 ruby lib

Gem::Specification.new do |s|
  s.name = "refinerycms-settings".freeze
  s.version = "4.0.1"

  s.required_rubygems_version = Gem::Requirement.new(">= 0".freeze) if s.respond_to? :required_rubygems_version=
  s.require_paths = ["lib".freeze]
  s.authors = ["Philip Arndt".freeze, "U\u0123is Ozols".freeze]
  s.cert_chain = ["-----BEGIN CERTIFICATE-----\\n5r7Wz/HY31SM47mkK21saPJG4NvUFEycf0wlpzP657Pl9aVo47aKKbxX\n-----END CERTIFICATE-----\n".freeze]
  s.date = "2018-02-11"
  s.description = "Adds programmer creatable, user editable settings.".freeze
  s.email = "info@refinerycms.com".freeze
  s.homepage = "http://refinerycms.com".freeze
  s.licenses = ["MIT".freeze]
  s.rubygems_version = "2.7.7".freeze
  s.summary = "Settings engine for Refinery CMS".freeze

  s.installed_by_version = "2.7.7" if s.respond_to? :installed_by_version

  if s.respond_to? :specification_version then
    s.specification_version = 4

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_runtime_dependency(%q<refinerycms-core>.freeze, [">= 4.0.0", "~> 4.0"])
      s.add_runtime_dependency(%q<friendly_id>.freeze, ["< 5.3", ">= 5.1.0"])
    else
      s.add_dependency(%q<refinerycms-core>.freeze, [">= 4.0.0", "~> 4.0"])
      s.add_dependency(%q<friendly_id>.freeze, ["< 5.3", ">= 5.1.0"])
    end
  else
    s.add_dependency(%q<refinerycms-core>.freeze, [">= 4.0.0", "~> 4.0"])
    s.add_dependency(%q<friendly_id>.freeze, ["< 5.3", ">= 5.1.0"])
  end
end
