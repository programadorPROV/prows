# -*- encoding: utf-8 -*-
# stub: refinerycms-images 4.0.1 ruby lib

Gem::Specification.new do |s|
  s.name = "refinerycms-images".freeze
  s.version = "4.0.1"

  s.required_rubygems_version = Gem::Requirement.new(">= 0".freeze) if s.respond_to? :required_rubygems_version=
  s.require_paths = ["lib".freeze]
  s.authors = ["Philip Arndt".freeze, "U\u0123is Ozols".freeze, "Rob Yurkowski".freeze]
  s.cert_chain = ["-----BEGIN CERTIFICATE-----\\n5r7Wz/HY31SM47mkK21saPJG4NvUFEycf0wlpzP657Pl9aVo47aKKbxX\n-----END CERTIFICATE-----\n".freeze]
  s.date = "2017-10-25"
  s.description = "Handles all image upload and processing functionality in Refinery CMS.".freeze
  s.email = "refinerycms@p.arndt.io".freeze
  s.homepage = "http://refinerycms.com".freeze
  s.licenses = ["MIT".freeze]
  s.required_ruby_version = Gem::Requirement.new(">= 2.2.2".freeze)
  s.rubyforge_project = "refinerycms".freeze
  s.rubygems_version = "2.7.7".freeze
  s.summary = "Images extension for Refinery CMS".freeze

  s.installed_by_version = "2.7.7" if s.respond_to? :installed_by_version

  if s.respond_to? :specification_version then
    s.specification_version = 4

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_runtime_dependency(%q<dragonfly>.freeze, [">= 1.1.0", "~> 1.1"])
      s.add_runtime_dependency(%q<globalize>.freeze, ["< 5.2", ">= 5.1.0.beta1"])
      s.add_runtime_dependency(%q<activemodel-serializers-xml>.freeze, [">= 1.0.1", "~> 1.0"])
      s.add_runtime_dependency(%q<refinerycms-core>.freeze, ["= 4.0.1"])
    else
      s.add_dependency(%q<dragonfly>.freeze, [">= 1.1.0", "~> 1.1"])
      s.add_dependency(%q<globalize>.freeze, ["< 5.2", ">= 5.1.0.beta1"])
      s.add_dependency(%q<activemodel-serializers-xml>.freeze, [">= 1.0.1", "~> 1.0"])
      s.add_dependency(%q<refinerycms-core>.freeze, ["= 4.0.1"])
    end
  else
    s.add_dependency(%q<dragonfly>.freeze, [">= 1.1.0", "~> 1.1"])
    s.add_dependency(%q<globalize>.freeze, ["< 5.2", ">= 5.1.0.beta1"])
    s.add_dependency(%q<activemodel-serializers-xml>.freeze, [">= 1.0.1", "~> 1.0"])
    s.add_dependency(%q<refinerycms-core>.freeze, ["= 4.0.1"])
  end
end
